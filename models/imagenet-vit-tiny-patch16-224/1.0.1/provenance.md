# imagenet-vit-tiny-patch16-224 1.0.0

**우리가 학습한 가중치가 아니다.** 남이 학습한 것을 옮겨 왔고, 이 파일이
그 출처다.

- 출처: `timm 1.0.28` 의 `vit_tiny_patch16_224`
- 체크포인트: `timm/vit_tiny_patch16_224.augreg_in21k_ft_in1k`
- 가중치 라이선스: apache-2.0
- 열쇠 152개 · 22,883,408 바이트
- sha256 `47f5bc05e27fa205967fda57525477010e225f73c4df9f92e066ab61dede0357`

## 수 — 무엇을 누가 쟀는지

매니페스트의 `metrics` 에 **우리가 잰 수**가 들어 있다. ImageNet-V2 에서
top-1 0.627, 표본 1000 장(클래스마다 한 장).

같은 사진에 timm 을 돌린 것이 0.625 다. 이 수가 옆에 있어야 위 수가
읽힌다 — 하나만 있으면 흔히 인용되는 ImageNet 점수와 견주게 되고,
**변환이 깨졌다는 결론**이 난다. 실제로 그 질문이 먼저 나왔다.

원저자가 발표한 ImageNet top-1 은 0.755 다. 매니페스트에는
