# imagenet-efficientnet-b3 1.0.0

**우리가 학습한 가중치가 아니다.** 남이 학습한 것을 옮겨 왔고, 이 파일이
그 출처다.

- 출처: `timm 1.0.28` 의 `efficientnet_b3`
- 체크포인트: `timm/efficientnet_b3.ra2_in1k`
- 가중치 라이선스: apache-2.0
- 열쇠 574개 · 49,335,792 바이트
- sha256 `e5d45273cc91c735aa99f042338febcabe0a323a1e1dd3b3c029d49de88124c0`

## 수 — 무엇을 누가 쟀는지

매니페스트의 `metrics` 에 **우리가 잰 수**가 들어 있다. ImageNet-V2 에서
top-1 0.711, 표본 1000 장(클래스마다 한 장).

같은 사진에 timm 을 돌린 것이 0.711 다. 이 수가 옆에 있어야 위 수가
...
짧은 변을 `329` 로 키운 뒤 가운데를
`[3, 288, 288]` 로 자른다(crop_pct 0.875).
보간은 `bicubic` — 원본과 같다.
