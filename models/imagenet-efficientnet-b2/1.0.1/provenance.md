# imagenet-efficientnet-b2 1.0.0

**우리가 학습한 가중치가 아니다.** 남이 학습한 것을 옮겨 왔고, 이 파일이
그 출처다.

- 출처: `timm 1.0.28` 의 `efficientnet_b2`
- 체크포인트: `timm/efficientnet_b2.ra_in1k`
- 가중치 라이선스: apache-2.0
- 열쇠 508개 · 36,757,484 바이트
- sha256 `471da2f475943f6fdf16b314cc7c059fdde0fa1db4ccd0d5f7a17065b6d41095`
...
같은 사진에 timm 을 돌린 것이 0.675 다. 이 수가 옆에 있어야 위 수가
읽힌다 — 하나만 있으면 흔히 인용되는 ImageNet 점수와 견주게 되고,
**변환이 깨졌다는 결론**이 난다. 실제로 그 질문이 먼저 나왔다.

원저자가 발표한 ImageNet top-1 은 0.804 다. 매니페스트에는
...
짧은 변을 `292` 로 키운 뒤 가운데를
`[3, 256, 256]` 로 자른다(crop_pct 0.875).
보간은 `bicubic` — 원본과 같다.
