# imagenet-mobilenetv3-large-100 1.0.0

**우리가 학습한 가중치가 아니다.** 남이 학습한 것을 옮겨 왔고, 이 파일이
그 출처다.

- 출처: `timm 1.0.28` 의 `mobilenetv3_large_100`
- 체크포인트: `timm/mobilenetv3_large_100.ra_in1k`
- 가중치 라이선스: apache-2.0
- 열쇠 312개 · 22,058,176 바이트
- sha256 `42568691e70638e69c380f871828bbc4131d9039e4e742ec8a476ef136dff2d5`

## 수 — 무엇을 누가 쟀는지

매니페스트의 `metrics` 에 **우리가 잰 수**가 들어 있다. ImageNet-V2 에서
top-1 0.639, 표본 1000 장(클래스마다 한 장).

같은 사진에 timm 을 돌린 것이 0.636 다. 이 수가 옆에 있어야 위 수가
읽힌다 — 하나만 있으면 흔히 인용되는 ImageNet 점수와 견주게 되고,
**변환이 깨졌다는 결론**이 난다. 실제로 그 질문이 먼저 나왔다.

원저자가 발표한 ImageNet top-1 은 0.758 다. 매니페스트에는
